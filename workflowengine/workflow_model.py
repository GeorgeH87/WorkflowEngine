...
class TaskExecution(BaseModel):
    ...
    def repeat(
        self,
        consider_datetime: datetime = datetime.now(),
        run_count: int = None
    ) -> Status:
...
class WorkflowModel(BaseModel):
    ...
    due_date: Optional[constr(regex="(>|<){0,1}[\d-]+")]
    ...
    def get_execution(self, execution) -> list[TaskExecution]:
        ...
        return list(sorted((
            execution for execution in (self.executions or [])
            if execution.name == name
        ), key=lambda e: e.execution_time, reverse=True))
    ...
    def has_processed(self, execution) -> list[TaskExecution]:
        ...
        """
        return [
            execution for execution in self.get_execution(execution)
