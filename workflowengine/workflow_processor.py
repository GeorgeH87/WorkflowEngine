...
from .workflow_model import WorkflowModel, TaskExecution
from .status_exception import StatusException
...
class WorkflowProcessor(ABC):
    """In case that the operator has not passed a list of specific
        subscriptions this method will be called to get the default list of
        subscriptions
    ...
    def __init__(self, consider_date: Union[datetime, str] = None):
        ...
        consider_date = consider_date or os.getenv(
            "consider_date", datetime.now())
    ...
    @abstractmethod
    def get_default_subscriptions(self) -> WorkflowModel:
        """retrieve a list of default subscription keys
    ...
    def filter_by_already_finished(
        self,
        entry: WorkflowModel
    ) -> bool:
        """Filter by status
        ...
        for index, task in enumerate(tasks):
            ...
            if len(executions) > 0:
                ...
                if execution.processed and not repeat:
                    ...
                    logging.info(
                        "task '%s' already processed for entry '%s'", 
                        task_name, entry)
                    continue
                ...
                if execution.error_id and not repeat:
                    ...
                    logging.info(
                        "task '%s' already failed for entry '%s'", 
                        task_name, entry)
                    return False
                ...
                if execution.status and not repeat:
                    ...
                    logging.info(
                        "task '%s' interrupted for '%s'", task_name, entry)
                    return False

                self.statistic_increment(*increment, 'repeat')
                logging.info(
                    "repeat task '%s' for entry '%s'", task_name, entry)
            ...
            try:
                ...
                logging.info(
                    "successfully processed task '%s' for entry '%s'", 
                    task_name, entry)
                ...
                logging.info(
                        "task '%s' has been skipped for entry '%s'", 
                        task_name, entry)
            ...
            if interrupt:
                ...
                logging.info(
                    "interrupted task '%s' for entry '%s'", 
                    task_name, entry)
                return False
    ...
    def process_subscriptions(
        ...
        """Process a list of subscriptions. We don't care about the content
        type of that list.

        :param subscriptions: a list of subscriptions, defaults to None
        :type subscriptions: list[any], optional
        :param limit: how many subscriptions should be processed,
            defaults to None
        ...
        if selections:
            selected_entries = [
                self.get_entry(selection) for selection in selections]
        else:
            with self.get_time_statistic('get_default_subscriptions'):
                selected_entries = self.get_default_subscriptions()
    ...
    def process(self, args: Namespace):
        ...
        :type args: Namespace
        """
        with self.get_time_statistic('process_subscriptions'):
            self.process_subscriptions(args.subs, args.limit, args.dryrun)
    ...
    def statistic_increment(self, *subs):
        ...
        .. code-block:
            self.statistic_increment('subscriptions', 'success')
            self.statistic_increment('subscriptions', 'success')
            self.statistic_increment('subscriptions', 'failed')
